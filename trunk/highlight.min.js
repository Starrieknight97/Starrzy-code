$(function(){
	...
	if(window.hljs){
		...
		$('pre code(.inline-editor pre)').each(function(i, e) {
	...
	$('pre .inline-editor').each(function(index){
		...
		window['inlineEditorMessage'+index] = function(arr, type){
			...
			if(type == 'error'){				
				...
				result.find('.inline-log').append('<div class="log">' + arr.join(' ').replace('<', '&lt;').replace('>', '&gt;') + '</div>');
		...
		// The Run button
		$('<a class="button blue">Run</a>').click(function(){
	...
	$('#newsletterSubscribe form').submit(function(e){
		...
		$.post(newsletterForm.attr('action'),{ email: $('#newsletterSubscribe input[type=text]').val() , submit:'1' },function(r){
			...
			if(r.match('JS:SUCCESS')){
				...
				errHolder.html('You are already registered. <a href="/members" target="_blank">Log in &raquo;</a>');
			}
			else {
				hider.stop().fadeTo('fast',0,function(){$(hider.hide())});
